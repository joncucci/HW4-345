...
if (process.env.NODE_ENV != 'test')
{
	(async () => {
		...
		var repo = "GEDCOM_database"
		await listBranches(userId, repo);
		await createRepo(userId,"newrepo");
		await createIssue(userId, "newrepo", "Bug Found", "I'm having an issue with this");
...
async function listBranches(owner,repo)
{
	...
	return new Promise(function(resolve, reject)
	{
		request(options, function (error, response, body) {
			...
			console.log(`Branches for repository: ${repo}`)
			for( var i = 0; i < obj.length; i++ )
			{
				var name = obj[i].name;
				console.log( name );
...
async function enableWikiSupport(owner,repo, bool = true)
{
	let options = getDefaultOptions(`/repos/${owner}/${repo}`, "PATCH");
	options.body = JSON.stringify({has_wiki: bool})
